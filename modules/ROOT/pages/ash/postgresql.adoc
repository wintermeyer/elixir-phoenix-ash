...
The database has been created but it is still empty. We need to
migrate it. Migrations are used to track changes in the database
layout.

Ash provides a "let me do it for you" command to generate migrations
for all resources in your application. It is called `mix ash.codegen`.
...
Let's add some more attributes to the `Product` resource:
...
For them to be added to the database we need to gnerate a
migration and than run it:
...
Generating Migrations:
* creating priv/repo/migrations/20231005155818_migrate_resources2.exs
stefan@Bimini app % mix ash_postgres.migrate
...
Just because we are courious we check the table again:
...
The table is created. Time to add some data.
...
INSERT INTO "products" ("id","name","price") VALUES ($1,$2,$3) RETURNING "price","name","id" ["4d7e383b-ce7b-44d0-818c-290eaa8b0532", "Banana", Decimal.new("0.1")]
...
INSERT INTO "products" ("id","name","price") VALUES ($1,$2,$3) RETURNING "price","name","id" ["e854a911-4cda-4693-bd49-db200b675ded", "Pineapple", Decimal.new("0.5")]
...
TIP: Never forget to run `mix ash.codegen` and `ash_postgres.migrate`
