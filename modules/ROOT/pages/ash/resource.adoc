...
In the Ash world a resource needs to be registered to an interal API.
Don't think of this as an external WebAPI. Ash uses the term API for
an internal programming interface. We call our API `ToDoList`.
...
Now we create the API module which contains the resource.
...
After the API we create the resource. For that we use `attributes`.
Attributes are the fields of the resource. In our case we have two
attributes: `id` and `content`. The `id` attribute is a special
attribute because it is the primary key of the resource. We use
the `uuid_primary_key` macro to define the `id` attribute as a primary
...
`App.ToDoList.Task.create(%{content: "Mow the lawn"})` is just a lot
more convenient.
====
