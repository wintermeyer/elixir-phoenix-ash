...
NOTE: This documentation covers the current Ash 3.x line (3.24+ at the
...
`Ash.Api` concept has been renamed to `Ash.Domain`. If you see a 2.x
tutorial online, translate it in your head before copy-pasting. For
example, `code_interface do define_for App.Shop end` in the resource is
now a `define :name, action: :create` line on the *domain*, and
`App.Shop.Product.create!(attrs)` has become
`App.Shop.create_product!(attrs)`.
