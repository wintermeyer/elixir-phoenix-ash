...
use xref:minimal-ash-2x-setup-guide.adoc[Minimal Ash 2.x Setup Guide] to generate
...
<<ash-relationship-setup, relationship setup>> code which has just the
`Product` resource. We add a `Tag` and a `ProductTag` resource which is
