...
example code which has just the `Product` resource. If you want to you
can delete the `Category` resource (`lib/app/shop/resources/category.ex`)
because we don't need it for this example.

We add a `Tag` resource and as a join table a `ProductTag` resource.
...
For the `Product` resource we add the `tags` relationship:
