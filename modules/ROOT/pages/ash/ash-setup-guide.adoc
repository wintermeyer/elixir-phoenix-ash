...
Ash 3.x is still a moving target. The examples in this book were
verified against the following versions:

* `ash` `~> 3.24` (3.24.3 at the time of writing)
* `ash_postgres` `~> 2.9` (2.9.0 at the time of writing)
* `ash_phoenix` `~> 2.3`
* `igniter_new` `~> 0.5` (the archive, plus `igniter` as a dep)
