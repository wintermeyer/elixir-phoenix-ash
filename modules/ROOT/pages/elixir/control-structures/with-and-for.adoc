...
`for` comprehensions in Elixir provide a way to iterate over
xref:elixir/enumerables.adoc[enumerables] (lists, maps, ranges, and similar
data structures, covered in a later chapter) and generate a new list,
optionally filtering the elements. The result of this operation can be
...
You can also filter elements in a `for` comprehension by adding a filter
expression after the generator. Elements for which the expression returns
`false` are skipped:
