...
with the xref:elixir/operators/pipe-operator.adoc[pipe operator] (`|>`), because
the printed value stays in the chain and the code keeps flowing.

NOTE: The `/2` after `dbg` is the function's *arity*, the number of arguments it
...
value, so it is optional. We cover xref:elixir/modules-and-functions.adoc#function-arity[arity]
