...
* *Huge collections* where computing every intermediate step would
eat too much memory.
* *Infinite sequences* (file lines, clock ticks, generated IDs), where
you only want the first few items and stopping early is essential.
...
NOTE: `Enum.reduce/3` is Elixir's version of what other languages call
...
More details can be found at the official link:https://hexdocs.pm/elixir/Stream.html#concat/2[Elixir Stream.concat/1,2 documentation].
