...
Interactive Elixir (1.20.0-rc.4) - press Ctrl+C to exit (...)
...
4.25
----
In these cases, we've applied a 20% discount to the price of apples (which was 10), and the result is 8.0. The result is a float because `/` always returns a float in Elixir, so the intermediate multiplication promotes the value. Similarly, we've applied a 15% discount to the price of bananas (which was 5), and the result is 4.25.
