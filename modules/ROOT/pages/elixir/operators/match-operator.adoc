...
operator*. Instead of "assign", read `=` as "try to match the left-hand
side against the right-hand side".

For simple cases the distinction does not matter much. When you write
`x = 5`, Elixir sees an unbound variable on the left and happily binds
`x` to `5`, which looks and feels like assignment in any other language:
...
Where it differs is when the left side already has a shape. Elixir then
*checks* that the shape matches, and binds any variable names that
appear inside. That is where pattern matching starts to earn its name.
