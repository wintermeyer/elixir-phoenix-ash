...
[...]
----
<1> `import Config` is the modern replacement for the deprecated
`use Mix.Config`. Phoenix 1.8 uses it everywhere.
...
* creating priv/repo/migrations/20260101120000_create_authors.exs <1>
...
* creating test/support/fixtures/store_fixtures.ex
* injecting test/support/fixtures/store_fixtures.ex

Remember to update your repository by running migrations:

    $ mix ecto.migrate
----
<1> The timestamp prefix on migration filenames reflects the moment
you run the generator, so yours will differ.
...
defmodule Bookstore.Repo.Migrations.CreateAuthors do
  ...
  def change do
    create table(:authors) do
      ...
      timestamps(type: :utc_datetime) <1>
    end

  end
end
----
<1> `timestamps(type: :utc_datetime)` generates the fields `updated_at`
and `inserted_at`. Both are set to "now" when a row is inserted; after
that only `updated_at` is bumped on each change. Since Phoenix 1.7 the
default timestamp type is `:utc_datetime`, which we pass explicitly for
clarity.
...
[info] Running BookstoreWeb.Endpoint with Bandit 1.10.4 at 127.0.0.1:4000 (http)
...
[debug] QUERY OK source="authors" db=2.5ms decode=0.4ms queue=0.5ms idle=1.9ms <1>
INSERT INTO "authors" ("first_name","last_name","twitter_account","inserted_at","updated_at") VALUES ($1,$2,$3,$4,$5) RETURNING "id" ["Dave", "Thomas", "pragdave", ~U[2026-04-18 18:28:48Z], ~U[2026-04-18 18:28:48Z]]
{:ok,
 %Bookstore.Store.Author{
   ...
   inserted_at: ~U[2026-04-18 18:28:48Z], <2>
   updated_at: ~U[2026-04-18 18:28:48Z]
 }}
----
<1> The `[debug] QUERY ...` lines show how Ecto processed the query. We
will not show this output in the other examples in this chapter.
<2> Phoenix 1.8 generates schemas with `:utc_datetime` timestamps, so
`inserted_at` and `updated_at` use the `~U[…Z]` (UTC) sigil. Older
Phoenix versions used `~N[...]` (NaiveDateTime), which is why you may
see that form in legacy code.
...
{:ok,
 %Bookstore.Store.Author{
   ...
   last_name: "Thomas",
   twitter_account: "pragdave",
   inserted_at: ~U[2026-04-18 18:28:48Z],
   updated_at: ~U[2026-04-18 18:28:48Z]
 }}
iex> author
%Bookstore.Store.Author{
  ...
  first_name: "Dave",
  id: 1,
  inserted_at: ~U[2020-05-27 11:00:19Z],
  last_name: "Thomas",
  twitter_account: "pragdave",
  updated_at: ~U[2020-05-27 11:00:19Z]
}
----
====
...
{:ok,
 %Bookstore.Store.Author{
   ...
   first_name: nil,
   id: 3,
   inserted_at: ~U[2020-05-27 05:37:46Z],
   last_name: "Thomas",
   twitter_account: "pragdave",
   updated_at: ~U[2020-05-27 05:37:46Z]
 }}
----
...
[.agentic-tip]
****
.Agentic Coding Tip: `cast` is the Allow-List — Don't Let the Agent Widen It
The `cast(attrs, [:first_name, :last_name, :twitter_account])`
call in the changeset is Ecto's allow-list. Only those three
fields can flow in from a form (or any other external data
source). Everything else — `id`, `inserted_at`, `role`,
`verified`, whatever you haven't named — gets silently
dropped. That is exactly what you want: it is the mechanism
that stops a hostile request from setting `admin: true` by
crafting the form submission.

When you ask Claude to "add a `featured` field to Author,"
it will correctly add the column, update the form, add
`:featured` to the `cast` list, and add a validation. That
is the safe path. When it hits friction — a test failure, a
missing-field error, a bug in an adjacent function — it will
sometimes reach for one of these shortcuts:

* `cast(attrs, __schema__(:fields))` — permit every field
  the schema defines.
* `cast(attrs, Map.keys(attrs))` — permit whatever the caller
  sent.
* Quietly adding admin-only fields (`role`, `verified`,
  `user_id`) to the cast list because a test "was failing."

Each of these is a real security hole. The first and second
turn the changeset into a rubber stamp for arbitrary input.

*Rule to add to your project's `CLAUDE.md`:*

[source]
----
Never pass a dynamic field list to `Ecto.Changeset.cast/4`.
Every field must be listed by name. Admin-only or
system-only columns (role, verified, user_id, confirmation
tokens, system status fields) stay out of the user-facing
`cast/4` call entirely, even if a test seems to demand them
— in that case the test needs fixing, not the allow-list.
Prefer a separate admin-only changeset function
(`admin_changeset/2`) when a privileged update path really
is needed.
----

When reviewing an agent's diff on a changeset, the fastest
check is to grep for `__schema__(:fields)`, `Map.keys`, and
any new `:role` / `:admin` / `:user_id` entries in a `cast`
list — and to challenge every one of them.
****
...
{:ok,
 %Bookstore.Store.Author{
   ...
   first_name: "Dave",
   id: 1,
   inserted_at: ~U[2020-05-27 06:17:18Z],
   last_name: "Thomas",
   twitter_account: "pragdave",
   updated_at: ~U[2020-05-27 06:17:18Z]
 }} <1>
iex> Bookstore.Store.create_author(%{first_name: "Dave", last_name: "Thomas", twitter_account: "pragdave"})
{:error,
 #Ecto.Changeset<
...
{:ok,
 %Bookstore.Store.Author{
   ...
   first_name: "Dave",
   id: 1,
   inserted_at: ~U[2020-05-27 08:35:29Z],
   last_name: "Thomas",
   twitter_account: nil,
   updated_at: ~U[2020-05-27 08:35:29Z]
 }}
...
[
  %Bookstore.Store.Author{
    ...
    first_name: "Dave",
    id: 1,
    inserted_at: ~U[2020-05-27 11:48:17Z],
    last_name: "Thomas",
    twitter_account: "pragdave",
    updated_at: ~U[2020-05-27 11:48:17Z]
  },
  %Bookstore.Store.Author{
    __meta__: #Ecto.Schema.Metadata<:loaded, "authors">,
    first_name: "James",
...
%Bookstore.Store.Author{
  ...
  first_name: "Dave",
  id: 1,
  inserted_at: ~U[2020-09-04 02:40:21Z],
  last_name: "Thomas",
  twitter_account: "pragdave",
  updated_at: ~U[2020-09-04 02:40:21Z]
}
...
{:ok,
 %Bookstore.Store.Author{
   ...
   last_name: "Thomas",
   twitter_account: nil,
   updated_at: ~U[2020-05-28 10:54:55Z]
 }}
...
{:ok,
 %Bookstore.Store.Book{
   ...
   author_id: 1,
   id: 4,
   inserted_at: ~U[2020-09-08 01:17:34Z],
   title: "Programming Elixir",
   updated_at: ~U[2020-09-08 01:17:34Z]
 }}
----
...
%Bookstore.Store.Author{
  ...
  first_name: "Dave",
  id: 1,
  inserted_at: ~U[2020-09-08 01:56:38Z],
  last_name: "Thomas",
  twitter_account: "pragdave",
  updated_at: ~U[2020-09-08 01:56:38Z]
}
...
[
  %Bookstore.Store.Book{
    ...
    author_id: 1,
    id: 1,
    inserted_at: ~U[2020-09-08 01:56:51Z],
    title: "Programming Elixir",
    updated_at: ~U[2020-09-08 01:56:51Z]
  }
]
...
[.agentic-tip]
****
.Agentic Coding Tip: N+1 Queries via Forgotten Preloads
The single most common Ecto bug an AI agent produces: it
writes a LiveView or template that iterates over records
and calls an association in the loop, without preloading
it. Every iteration fires a fresh `SELECT` and you don't
notice on seed data.

[source,heex]
----
<%! WRONG — one query per book %>
<%= for book <- @books do %>
  <p><%= book.title %> — <%= book.author.last_name %></p>
<% end %>
----

The `book.author` on each iteration triggers a database
lookup *unless* the association was preloaded. With twenty
books you pay twenty-one queries instead of two. The page
still works. It falls over when the table grows.

The fix is a preload in the context function that supplies
the data:
...
# Or, in a single query joined at the database:
...
Agents don't naturally check for this because the code
runs clean in development. The pitfall only shows up in
the logs (`[debug] QUERY ...` lines repeating per record).

*Rule to add to your project's `CLAUDE.md`:*

[source]
----
When generating or editing a LiveView, template, or
context function that iterates over records and touches
an association (book.author, post.comments,
user.organization, etc.), add a `Repo.preload/2` or a
`preload:` clause in the Ecto query. After making the
change, run the page / action in iex or the browser,
inspect the emitted SQL in `:debug` logs, and confirm
there is one query per association, not one per record.
----

The same applies to `assoc()` in queries: `Repo.one(...) |>
Repo.preload([:author, comments: :user])` when the view
needs nested associations.
****

### Where to go next

Ecto has many features we did not cover here: schemaless changesets,
the full `Ecto.Query` DSL, multi-step transactions with `Ecto.Multi`,
custom types, and more. Two recommended resources:

* The official https://hexdocs.pm/ecto[Ecto documentation].
* The book https://pragprog.com/book/wmecto/programming-ecto[Programming
Ecto] by Darin Wilson and Eric Meadows-Jönsson.

When you are ready to move beyond hand-written contexts and
changesets, jump to the xref:ash/index.adoc[Ash Framework chapter],
which builds on top of Ecto and makes resource definitions more
declarative.
