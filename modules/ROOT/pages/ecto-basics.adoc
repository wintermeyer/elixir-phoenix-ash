...
[IMPORTANT] 
====
We are going to use a https://www.postgresql.org[PostgreSQL]
...
this book we assume that you have a PostgreSQL user on your development which was 
granted the right to create a database and which has the following credentials:

- user: postgres
- password: postgres

Simply because those are the default values for the development system in Phoenix.
====
...
The migration created that table and added columns. But it added more columns than 
we asked for. It added `id`, `inserted_at` and `updated_at`. But that is all right. 
We will see what they are for in a minute.
